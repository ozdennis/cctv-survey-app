...
export default function CameraSettingsPage() {
    ...
    const [newCamera, setNewCamera] = useState({ name: "", type: "Equipment", category: "CCTV", resolution: "", features: "" });
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        if (!error && data) {
            ...
            setNewCamera({ name: "", type: "Equipment", category: "CCTV", resolution: "", features: "" });
            setIsAdding(false);
        } else {
        ...
        <div className="max-w-6xl mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-400">Manage all scraped equipment, cameras, and materials. Hover over items to see details.</p>
                </div>
                <button
                ...
                <form onSubmit={handleAdd} className="bg-slate-900 border border-slate-800 shadow-lg rounded-xl p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <select value={newCamera.category} onChange={e => setNewCamera({ ...newCamera, category: e.target.value })} className="w-full px-4 py-2 bg-slate-800 border border-slate-700 text-white rounded-lg focus:ring-2 focus:ring-primary outline-none">
                                <option>CCTV</option>
                                ...
                                <tr key={camera.id} className="hover:bg-slate-800/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="relative group flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-primary border border-slate-700 cursor-help">
                                                <CameraIcon className="w-4 h-4" />
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="font-medium text-white cursor-help border-b border-dashed border-slate-500 whitespace-nowrap overflow-hidden text-ellipsis max-w-[250px]">{camera.name}</span>
                                            </div>

                                            {/* Hover Tooltip */}
                                            {camera.features && (
                                                <div className="absolute left-[20%] bottom-full mb-2 w-80 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 bg-slate-800 border border-slate-700 shadow-2xl rounded-lg p-4 pointer-events-none">
                                                    <h4 className="text-sm font-bold text-white mb-2">{camera.name}</h4>
                                                    <div className="text-xs text-slate-300 whitespace-pre-wrap leading-relaxed">{camera.features}</div>
                                                    <div className="absolute left-1/4 top-full -translate-x-1/2 border-8 border-transparent border-t-slate-700"></div>
                                                </div>
                                            )}
                                        </div>
                                    </td>
